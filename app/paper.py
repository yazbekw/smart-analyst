...
def open_paper_trade(signal_result, capital=10000, risk_pct=1.0):
    """يفتح صفقة ورقية عند إشارة جديدة"""
...
def check_paper_trades(current_prices: dict):
    """يفحص الصفقات المفتوحة ويحدّثها"""
    try:
    ...
    for t in trades:
        ...
        if t["direction"] == "LONG":
            if price <= t["stop_loss"]:
                hit, exit_price = "sl_hit", t["stop_loss"]
            elif price >= t["tp3"]:
                hit, exit_price = "tp3_hit", t["tp3"]
            elif price >= t["tp2"]:
                hit, exit_price = "tp2_hit", t["tp2"]
            elif price >= t["tp1"]:
                hit, exit_price = "tp1_hit", t["tp1"]
        else:
            if price >= t["stop_loss"]:
                hit, exit_price = "sl_hit", t["stop_loss"]
            elif price <= t["tp3"]:
        ...
        if hit:
            if t["direction"] == "LONG":
                pnl = (exit_price - t["entry_price"]) * t["size"]
            else:
                pnl = (t["entry_price"] - exit_price) * t["size"]
            ...
            try:
                ...
                closed.append({**t, "hit": hit, "pnl": pnl})
            except Exception as e:
