...
def calculate_levels(df_15m, state, score):
    """حساب Entry/SL/TP باستخدام ATR"""
...
def analyze_symbol(symbol: str, df_btc=None) -> dict:
    ...
    for df, tf in [(df_4h, "4H"), (df_1h, "1H")]:
        s, r = sig_price_above_ema200(df)
        score += s; breakdown["trend"] += s
        if r: reasons.append(f"[{tf}] {r}")

        s, r = sig_ema50_above_ema200(df)
        score += s; breakdown["trend"] += s
        if r and tf == "1H": reasons.append(f"[{tf}] {r}")

        s, r = sig_ema50_slope_up(df)
        score += s; breakdown["trend"] += s
        if r and tf == "1H": reasons.append(f"[{tf}] {r}")

    s, r = sig_price_above_ema20(df_15m)
    score += s; breakdown["trend"] += s
    if r: reasons.append(f"[15M] {r}")

    s, r = sig_adx_strength(df_1h)
    score += s; breakdown["trend"] += s
    ...
    # ----- Volume -----
    s, r = sig_volume_ratio(df_15m)
    score += s; breakdown["volume"] += s
    if r: reasons.append(f"[15M] {r}")

    s, r = sig_volume_price_agreement(df_15m)
    score += s; breakdown["volume"] += s
    if r: reasons.append(f"[15M] {r}")

    # ----- Order Flow -----
    s, r = sig_orderbook_imbalance(ob)
    score += s; breakdown["orderflow"] += s
    if r: reasons.append(r)

    s, r = sig_taker_buy_pressure(trades)
    score += s; breakdown["orderflow"] += s
    if r: reasons.append(r)

    # ----- Structure -----
    s, r = sig_broke_resistance(df_15m)
    score += s; breakdown["structure"] += s
    if r: reasons.append(f"[15M] {r}")

    s, r = sig_near_support(df_15m)
    score += s; breakdown["structure"] += s
    if r: reasons.append(f"[15M] {r}")

    # ----- Context -----
    if df_btc is not None and symbol != BTC_REFERENCE:
        s, r = sig_btc_trend(df_btc)
        score += s; breakdown["context"] += s
        if r: reasons.append(r)

        s, r = sig_relative_strength(df_15m, df_btc)
        score += s; breakdown["context"] += s
        if r: reasons.append(r)
    ...
    # ----- Risk + Levels -----
    levels = calculate_levels(df_15m, state, score) if state != "NO TRADE" else None

    if levels:
        s, r = sig_rr_ratio(df_15m["close"].iloc[-1], levels["stop_loss"], levels["tp1"])
        score += s; breakdown["risk"] += s
        if r:
            if s < 0: warnings.append(r)
            else: reasons.append(r)

    s, r = sig_resistance_close(df_15m)
    score += s; breakdown["risk"] += s
    if r: warnings.append(r)

    # إعادة تقييم الحالة بعد Risk
    if score >= 15: state = "STRONG BUY SETUP"
    elif score >= 8: state = "BUY SETUP"
    elif score >= 3: state = "WATCH"
    elif score <= -15: state = "STRONG SELL SETUP"
    elif score <= -8: state = "SELL SETUP"
    else: state = "NO TRADE"
    ...
    save_snapshot({
        ...
        "state": state,
        "details": {"reasons": reasons, "warnings": warnings, "levels": levels},
    })
